...
## 创建标签容器
## holder: 容器所属的对象，用于自动注册
func create_tag_container(holder: Object = null) -> GameplayTagContainer:
	var container := GameplayTagContainer.new()
	if holder:
		container.holder = holder
		container.tag_added.connect(_on_container_tag_added.bind(holder))
		container.tag_removed.connect(_on_container_tag_removed.bind(holder))
		_object_to_tags[holder.get_instance_id()] = weakref(container)
	return container

## 从字符串数组创建标签容器
func create_tag_container_from_strings(tag_strings: Array, holder: Object = null) -> GameplayTagContainer:
	var container := create_tag_container(holder)
